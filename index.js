...
function hoursWorkedOnDate(employeeRecord, dateWorked) {
    const punchIn = employeeRecord.timeInEvents.find(employee => employee.date === dateWorked)

    const punchOut = employeeRecord.timeOutEvents.find(employee =>  employee.date === dateWorked)

    return (punchOut.hour - punchIn.hour) / 100
}

function wagesEarnedOnDate(employeeRecord, dateWorked) {
    const wagesOweForDay = hoursWorkedOnDate(employeeRecord, dateWorked) * employeeRecord.payPerHour
    return parseFloat(wagesOweForDay.toString())
}

function allWagesFor(employeeRecord) {
    let datesWorked = employeeRecord.timeInEvents.map(employee => employee.date)
